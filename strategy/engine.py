...
from gold_bolt_server.strategy.ai_analyzer import AIAnalyzer
...
class StrategyEngine:
    ...
    def analyze(self, dm: DataManager, account: dict, use_ai: bool = True) -> tuple:
        """
        ...
        # AI 智能分析（仅在 use_ai=True 时使用缓存结果做加权）
        ai_result = {}
        if use_ai:
            ai_result = self.ai.last_result or {}
            if ai_result:
                ai_bias = ai_result.get("bias", "neutral")
                ai_conf = ai_result.get("confidence", 0)
                ai_text = ai_result.get("analysis", "")
                ai_exit = ai_result.get("exit_suggestion", "hold")
                ai_warn = ai_result.get("risk_warning", "")
                
                msg = f"🤖 AI: {ai_bias}({ai_conf}%) | {ai_text}"
                if ai_warn:
                    msg += f" | ⚠️{ai_warn}"
                if ai_exit != "hold":
                    msg += f" | 出场建议: {ai_exit}"
                
                logs.append({"level": "info", "strategy": "AI分析", "msg": msg})
                
                key_levels = ai_result.get("key_levels", [])
                if key_levels:
                    logs.append({"level": "info", "strategy": "AI分析", "msg": f"关键价位: {key_levels}"})
        ...
        for sig in signals:
            ...
            ai_conf = ai_result.get("confidence", 0)
            if ai_conf >= 60:
                if (sig["side"] == "BUY" and ai_bias == "bullish") or \
                   (sig["side"] == "SELL" and ai_bias == "bearish"):
                    sig["score"] = min(sig["score"] + 1, 10)
                    logs.append({"level": "info", "strategy": "AI加权", "msg": f"AI {ai_bias} 确认 {sig['side']}，评分+1 → {sig['score']}"})
                elif (sig["side"] == "BUY" and ai_bias == "bearish") or \
                     (sig["side"] == "SELL" and ai_bias == "bullish"):
                    sig["score"] = max(sig["score"] - 1, 0)
                    logs.append({"level": "warn", "strategy": "AI加权", "msg": f"AI {ai_bias} 与 {sig['side']} 矛盾，评分-1 → {sig['score']}"})
    ...
    def run_ai_only(self, dm: DataManager, account: dict) -> list:
        """
        独立 AI 分析（由 ai_analysis_loop 调用）
        流程：收集多周期技术面 → 发给 AI → 分周期研判 → 合并最终策略
        返回日志列表
        """
        logs = []
        
        result = self.ai.analyze_with_technicals(dm, account)
        
        if not result:
            logs.append({"level": "info", "strategy": "AI分析",
                "msg": "🤖 AI 分析超时或不可用，跳过"})
            return logs
        
        # 分周期输出
        for tf in ["M15", "M30", "H1"]:
            tf_result = result.get(tf)
            if tf_result:
                bias = tf_result.get("bias", "neutral")
                conf = tf_result.get("confidence", 0)
                summary = tf_result.get("summary", "")
                emoji = "🟢" if bias == "bullish" else "🔴" if bias == "bearish" else "⚪"
                logs.append({"level": "info", "strategy": f"AI·{tf}",
                    "msg": f"{emoji} {bias}({conf}%) | {summary}"})
        ...
        if combined:
            ...
            key_levels = combined.get("key_levels", [])
            risk_warn = combined.get("risk_warning", "")
            
            emoji = "🟢" if bias == "bullish" else "🔴" if bias == "bearish" else "⚪"
            logs.append({"level": "signal", "strategy": "AI·综合",
                "msg": f"{emoji} {bias}({conf}%) | {analysis}"})
            
            if exit_sug != "hold":
                logs.append({"level": "warn", "strategy": "AI·综合",
                    "msg": f"📋 出场建议: {exit_sug} | {exit_reason}"})
            
            if key_levels:
                logs.append({"level": "info", "strategy": "AI·综合",
                    "msg": f"📍 关键价位: {', '.join(str(l) for l in key_levels)}"})
            ...
            # 更新缓存供技术分析加权用
            self.ai.last_result = combined
            self.ai.last_analysis_time = time.time()
