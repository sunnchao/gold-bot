...
func TestSendReturnsWithoutWaitingForWebhook(t *testing.T) {
	...
	notifier := New("https://example.invalid/webhook", &http.Client{Transport: transport})
	notifier.Cooldown = time.Minute

	start := time.Now()
	err := notifier.Send(context.Background(), map[string]any{"content": "hello"})
	if err != nil {
	...
	err = notifier.Send(context.Background(), map[string]any{"content": "hello again"})
	if err != nil {
